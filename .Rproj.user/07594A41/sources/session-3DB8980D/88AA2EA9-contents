...
# Word 1= arose -----
 ...
 ind3 <- 1
 for (ind3 in 1:nrow(five_words)) {
   test<- five_words[[ind3,"word_name"]]
   assert_is_character(test)
   print(test)
   assert_is_character(fivewords_word_1)
   
   
   five_words[[ind3, "word_guess2"]]<- Removing_Letters(test, fivewords_word_1,str_length(fivewords_word_1) )
   #print(word_scores[[ind3, "word_guess2"]])
   score_ind3 <- Scoring_Word_Unique( five_words[[ind3, "word_guess2"]])
   five_words[[ind3,"score_guess2"]] <- score_ind3
   
 }

top_words <- five_words %>%
   arrange(desc(score_guess2))

fivewords_word_2 <- top_words$word_name[1]

 
# This is for GUESS 3
ind3 <- 1
for (ind3 in 1:nrow(five_words)) {
  test<- five_words[[ind3,"word_guess2"]]
  assert_is_character(test)
  print(test)
  assert_is_character(fivewords_word_2)
  
  
  five_words[[ind3, "word_guess3"]]<- Removing_Letters(test, fivewords_word_2,str_length(fivewords_word_2) )
  #print(word_scores[[ind3, "word_guess3"]])
  score_ind3 <- Scoring_Word_Unique( five_words[[ind3, "word_guess3"]])
  five_words[[ind3,"score_guess3"]] <- score_ind3
...
fivewords_word_3 <- top_words$word_name[1]

# #this is for GUESS 4
ind3 <- 1
for (ind3 in 1:nrow(five_words)) {
  test<- five_words[[ind3,"word_guess3"]]
  assert_is_character(test)
  print(test)
  assert_is_character(fivewords_word_3)
  
  
  five_words[[ind3, "word_guess4"]]<- Removing_Letters(test, fivewords_word_3,str_length(fivewords_word_3) )
  #print(word_scores[[ind3, "word_guess2"]])
  score_ind3 <- Scoring_Word_Unique( five_words[[ind3, "word_guess4"]])
  five_words[[ind3,"score_guess4"]] <- score_ind3
...
#fivewords_word_4 <- top_words$word_name[1]
